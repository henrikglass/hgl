...
static inline HglRitaColor my_normal_map_shader(const HglRitaContext *ctx, const HglRitaFragment *in)
{
    (void) ctx;
...
int main()
{
    ...
    //hgl_rita_use_texture_wrapping(HGL_RITA_REPEAT);
   
    /* bind our vertex displacement shader */
    hgl_rita_bind_vert_shader(my_vertex_displacement_shader);
    hgl_rita_bind_frag_shader(my_normal_map_shader);
    
    HglRitaTexture displacement_map = load_texture("assets/heightmap.png");
    hgl_rita_bind_texture(HGL_RITA_TEX_DISPLACEMENT, &displacement_map);
    HglRitaTexture normal_map = load_texture("assets/normalmap.png");
