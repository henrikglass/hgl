...
TEST(test_vec2)
{
    ...
    Vec2 u = vec2(5, 5);
    Vec2 t = vec2(5, -5);
    ASSERT(vec2_eq(hglm_vec2_add(v, u), vec2(15, 25)));
    ASSERT(vec2_eq(hglm_vec2_sub(v, u), vec2(5, 15)));
    ASSERT(float_eq(hglm_vec2_distance(v, u), sqrtf(5*5+15*15)));
    ASSERT(float_eq(hglm_vec2_length(v), sqrtf(10*10+20*20)));
    ASSERT(vec2_eq(hglm_vec2_normalize(u), vec2(1/sqrtf(2), 1/sqrtf(2))));
    ASSERT(float_eq(hglm_vec2_dot(u, t), 0.0f));
    ASSERT(float_eq(hglm_vec2_dot(u, u), 50.0f));
    ASSERT(vec2_eq(hglm_vec2_hadamard(v, u), vec2(50, 100)));
    ASSERT(vec2_eq(hglm_vec2_mul_scalar(v, 0.5f), vec2(5, 10)));
    ASSERT(vec2_eq(hglm_vec2_reflect(t, vec2(0, 1)), u));
    ASSERT(vec2_eq(hglm_vec2_lerp(u, t, 0.5f), vec2(5, 0)));
    ASSERT(vec2_eq(hglm_vec2_bezier3(vec2(0,0), 
                                     vec2(0,-1),
                                     vec2(1,1),
                                     vec2(1,0), 0.0f), vec2(0, 0)));
    ASSERT(float_eq(hglm_vec2_bezier3(vec2(0,0), 
                                     vec2(0,-1),
                                     vec2(1,1),
                                     vec2(1,0), 0.5f).x, 0.5f));
    ASSERT(vec2_eq(hglm_vec2_bezier3(vec2(0,0), 
                                     vec2(0,-1),
                                     vec2(1,1),
                                     vec2(1,0), 1.0f), vec2(1, 0)));
}

TEST(test_ortho_inv)
{
    Mat4 M;
    Mat4 m = mat4_ortho(-5, 10, -10, 15, -2, 4);
    Mat4 m_inv = mat4_ortho_inverse(-5, 10, -10, 15, -2, 4);
...
TEST(test_determinants)
{
    ...
    ASSERT(float_neq(mat2_determinant(m2), 0.0f));

    Mat3 m3 = mat3_identity();
    ASSERT(float_neq(mat3_determinant(m3), 0.0f));
}

TEST(test_inverse_matrices)
{
    Mat2 m2 = mat2_identity();
    Mat2 m2_inv = mat2_inverse(m2);

    ASSERT(mat2_eq(m2, m2_inv));

    Mat3 m3 = mat3_identity();
    Mat3 m3_inv = mat3_inverse(m3);
    ...
    m3_inv = mat3_inverse(m3);
    ASSERT(float_neq(mat3_determinant(m3), 0.0f));
    ASSERT(mat3_eq(mat3_mul_mat3(m3, m3_inv), MAT3_IDENTITY));

    Mat4 m4 = mat4_identity();
    m4 = mat4_translate(m4, vec3(3, 5, 7));
    m4 = mat4_rotate(m4, 3.14f/5.0f, vec3_normalize(vec3(0, 1, 1)));
    m4 = mat4_scale(m4, vec3(2, 3, 4));
    mat4_log(m4);
    Mat4 m4_inv = mat4_affine_inverse(m4);
    mat4_log(m4_inv);
